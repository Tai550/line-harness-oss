...
export default function AutomationsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">オートメーション</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-6 space-y-3">
        ...
        <div>
          <label className="text-xs text-gray-500">条件 (JSON)</label>
        ...
        <div>
          <label className="text-xs text-gray-500">アクション (JSON)</label>
            ...
            <div key={a.id} className="bg-white rounded-xl shadow-sm p-4 flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs bg-purple-50 text-purple-600 px-2 py-0.5 rounded">{a.trigger_event}</span>
                  <span className={`text-xs px-2 py-0.5 rounded ${a.is_active ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-500"}`}>{a.is_active ? "有効" : "無効"}</span>
                </div>
              </div>
              <div className="flex gap-2">
                <a href={`/automations/detail?id=${a.id}`} className="text-sm text-blue-500 hover:underline">編集</a>
                <button onClick={() => toggle(a)} className="text-sm text-gray-500 hover:text-gray-700">{a.is_active ? "無効化" : "有効化"}</button>
                <button onClick={() => del(a.id)} className="text-sm text-red-400 hover:text-red-600">削除</button>
              </div>
            </div>
          ))}
        </div>
