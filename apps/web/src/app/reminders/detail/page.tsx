...
export default function ReminderDetailPage() {
  ...
  if (!Number.isFinite(reminderId)) {
    return <div className="text-sm text-brand-alert">リマインダーIDが不正です。</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-brand-black">リマインダー詳細</h1>
          <p className="text-sm text-brand-gray">本体編集、ステップ管理、友だちへの enroll を行います。</p>
        </div>
        <Link href="/reminders" className="text-sm text-brand-orange hover:text-brand-blue">
          一覧へ戻る
        </Link>
      </div>
      ...
      {reminder ? (
        <>
          <div className="bg-white rounded-[8px] shadow-sm p-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded text-sm ${reminder.is_active ? "bg-brand-orange/8 text-brand-orange" : "bg-brand-lightgray text-brand-black/60"}`}
              >
          ...
          <div className="bg-white rounded-[8px] shadow-sm p-4 space-y-4">
            ...
            {reminder.steps.length === 0 ? <p className="text-sm text-brand-gray">まだステップがありません。</p> : null}
              ...
              <div key={step.id} className="rounded-[8px] border border-brand-lightgray/70 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-brand-gray/70">{step.offset_minutes} 分</span>
                  </div>
                  <button
                    onClick={async () => {
                    ...
                    className="text-sm text-brand-alert hover:text-brand-alert"
                  >
                    削除
                  </button>
                </div>
          ...
          <div className="bg-white rounded-[8px] shadow-sm p-4 space-y-3">
            <h2 className="font-semibold text-brand-black">友だちへ enroll</h2>
