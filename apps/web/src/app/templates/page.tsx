...
export default function TemplatesPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">テンプレート</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-6 space-y-3">
        ...
        <button onClick={create} className="bg-green-500 text-white px-4 py-2 rounded text-sm hover:bg-green-600">保存</button>
      </div>

      <div className="space-y-3">
        {templates.map((t) => (
          <div key={t.id} className="bg-white rounded-xl shadow-sm p-4 flex justify-between">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-medium">{t.name}</h3>
                {t.category && <span className="text-xs bg-yellow-50 text-yellow-600 px-2 py-0.5 rounded">{t.category}</span>}
              </div>
              <p className="text-sm text-gray-500 mt-1 line-clamp-2">{t.content}</p>
            </div>
            <div className="ml-4 shrink-0 flex gap-3">
              <a href={`/templates/detail?id=${t.id}`} className="text-sm text-blue-500 hover:underline">編集</a>
              <button onClick={() => del(t.id)} className="text-sm text-red-400 hover:text-red-600">削除</button>
            </div>
          </div>
        ))}
      </div>
